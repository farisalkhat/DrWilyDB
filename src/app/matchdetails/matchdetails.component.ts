...
export class MatchdetailsComponent implements OnInit {
  ...
  getMatchDetails(){
    this.route.paramMap.subscribe((paramMap)=>{
      ...
      this.http.get<Match>(`https://mm8bitdm-v2.herokuapp.com/api/matches/${this.matchid}`).pipe(
      map(res=>{
        this.matchDetails = res
        return res
      }),
      mergeMap(res=>{
        const stageData = this.http.get<Stage>(`https://mm8bitdm-v2.herokuapp.com/api/stages/stageid/${res['stage']}`);
