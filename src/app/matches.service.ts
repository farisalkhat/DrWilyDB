...
export class MatchesService {
  ...
  private _matchesUrl = "https://mm8bitdm-ygo.herokuapp.com/api/matches"
  ...
  getRandomMatch(){
    return this.http.get<rmMatch>("https://mm8bitdm-ygo.herokuapp.com/api/randommatch");
  }

  getMostRecentMatch(){
    return this.http.get<Match>("https://mm8bitdm-ygo.herokuapp.com/api/recentmatch");
  }
  ...
  getFilteredMatches(data){
    return this.http.post<Match[]>('https://mm8bitdm-ygo.herokuapp.com/api/matches/filtered',data,);
  }

  getFilteredPlayerMatches(data){
    return this.http.post<Match[]>('https://mm8bitdm-ygo.herokuapp.com/api/matches/filteredplayermatches',data,);
  }

  deleteMatch(matchid){
    if (this.authGuard.canActivate()){
      return this.http.post<any>('https://mm8bitdm-ygo.herokuapp.com/api/deletematch',matchid)
    }
  ...
  getMatchDetails(matchid:string){
    ...
    return this.http.get<any[]>(`https://mm8bitdm-ygo.herokuapp.com/api/matches/${matchid}`).pipe(
      map(res=>{
        this.matchDetails = res
        return res
      }),
      mergeMap(res=>{
        const stageData = this.http.get<any[]>(`https://mm8bitdm-ygo.herokuapp.com/api/stages/stageid/${res['stage']}`);
        const playedMatchData = this.http.get<any[]>(`https://mm8bitdm-ygo.herokuapp.com/api/playedmatch/${res['matchid']}`);
